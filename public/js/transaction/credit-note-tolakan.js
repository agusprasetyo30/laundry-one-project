...
$(document).ready(function() {
    table = $('#credit-note-tolakan-table').DataTable({
...
$("input[name=credit_note_search]").on('keypress', function(e) {
    if (e.which === 13) { // 13 = Enter
        ...
        tableReload(false) // reload table & configuration
    }
})

$('#range-tanggal-creditnote-tolakan').daterangepicker({
    ...
    $('#range-tanggal-creditnote-tolakan').val(start.format('DD/MM/YYYY') + ' - ' + end.format('DD/MM/YYYY'));
...
function loadLazyData() {
    ...
    $.ajax({
        url: $('#credit-note-tolakan-table').data('table-route'),
        method: 'GET',
        data: { 
            page: page, 
            limit: limit,
...
$('#credit-note-tolakan-table').on('page.dt', function () {
...
$('#credit-note-tolakan-table').on('length.dt', function (e, settings, len) {
...
function tableReload(dataState = true, lastPageInput = false) {
    ...
    // jadi dari sini dibuat dinamis, dengan default true untuk tsidak kembali ke index 0
    if (dataState) {
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                data: {
                    ...
                    creditNoteType: 'tolakan'
                },
                type: "POST",
                url: $('#credit-note-tolakan-table').data('send-to-reckitt-route'),
