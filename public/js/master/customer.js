...
$(document).ready(function() {
    table = $('#master_customer_table').DataTable({
        ...
        columns: [
            ...
            { data: 'Kode', name: 'c.Kode' },
            { data: 'Group1', name: 'c.Group1' },
            { data: 'Perusahaan', name: 'c.Perusahaan' },
            { data: 'Alamat', name: 'c.Alamat' },
            { data: 'AliasKodeSegment', name: 's.AliasKodeSegment' },
            { data: 'Segment', name: 's.Description' },
            { data: 'KodeSales', name: 'KodeSales' },
            { data: 'Sales', name: 'sp.Nama' },
...
$("input[name=master_customer_search]").on('keypress', function(e) {
...
// digunakan untuk menampilkan data lazyload
function loadLazyData() {
    ...
    $.ajax({
        url: $('#master_customer_table').data('table-route'),
        method: 'GET',
        data: { 
            page: page, 
            limit: limit,
...
$('#master_customer_table').on('page.dt', function () {
    ...
    if ((info.page + 1) * info.length >= table.rows().count()) {
        ...
        limitIndex = limitIndex + 1; // digunakan untuk menambahkan limitIndex untuk konfigurasi jumlah limit yang tampilt
        tableReload()
    }
});

$('#master_customer_table').on('length.dt', function (e, settings, len) {
